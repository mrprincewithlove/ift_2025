    ...
    @if(count($activitylogs)>0)
        @foreach($activitylogs as $r)
            ...
            <tr class="intro-x" style="z-index: 1 !important;" >
                <td class="text-center">{{ $loop->iteration + $activitylogs->firstItem() - 1 }}</td>
                <td class="text-center">{{$r->log_name}}</td>
                <td class="text-center">{{$r->description}}</td>
                <td class="text-center flex items-center justify-center gap-1">
                    {{\Helper::splitModel($r->subject_type)}}
                ...
                <td class="text-center">{{($r->causer_id)? $r->user->getFullName() : ''}}
