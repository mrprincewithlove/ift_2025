	...
	<div class="grid grid-cols-12 gap-6 mt-5">
		<div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
			...
			<div class="hidden md:block mx-auto text-slate-500">
				{{ __('translates.entries_info', ['start' => $cities->firstItem(), 'end' => $cities->lastItem(), 'total' => $cities->total()]) }}
			</div>
		...
		<div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
			<table class="table table-report -mt-2">
				...
				<tr>
					...
					<th class="text-center whitespace-nowrap">{{__('translates.welayat_name')}}</th>
				...
				@if(count($cities)>0)
					@foreach($cities as $r)
						<tr class="intro-x">
							<td class="text-center">{{ $loop->iteration + $cities->firstItem() - 1 }}</td>
							<td class="text-center">{{ $r->name_tm ?? '' }}</td>
							<td class="text-center">{{ $r->name_ru ?? '' }}</td>
							<td class="text-center">{{ $r->name_en ?? '' }}</td>
							<td class="text-center">{{ ($r->welayat->{'name_'.app()->currentLocale() }) ? ($r->welayat->{'name_'.app()->currentLocale() })  : '' }}</td>
							...
							<td class="table-report__action w-56">
								<div class="flex justify-center items-center">
									<a class="flex items-center mr-3" href="{{route('cities.edit', $r)}}">
