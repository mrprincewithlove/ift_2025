...
@section('content')
	<h2 class="intro-y text-lg font-medium mt-10">
		{{__('translates.microdistricts')}}
	</h2>

	<div class="grid grid-cols-12 gap-6 mt-5">
		<div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
			<a href="{{route('microdistricts.create')}}"
			   class="btn btn-primary shadow-md mr-2">{{ __('translates.add_new') }}</a>

			<div class="hidden md:block mx-auto text-slate-500">
				{{ __('translates.entries_info', ['start' => $microdistricts->firstItem(), 'end' => $microdistricts->lastItem(), 'total' => $microdistricts->total()]) }}
			</div>

			<form action="{{route('microdistricts.index')}}" method="GET"
		...
		<div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
			<table class="table table-report -mt-2">
				...
				<tr>
					...
					<th class="text-center whitespace-nowrap">{{__('translates.city_name')}} ({{__('translates.welayat_name')}})</th>
				...
				<tbody>
				@if(count($microdistricts)>0)
					@foreach($microdistricts as $r)
						<tr class="intro-x">
							<td class="text-center">{{ $loop->iteration + $microdistricts->firstItem() - 1 }}</td>
							<td class="text-center">{{ $r->name_tm ?? '' }}</td>
							<td class="text-center">{{ $r->name_ru ?? '' }}</td>
							<td class="text-center">{{ $r->name_en ?? '' }}</td>
							<td class="text-center">{{ ($r->city->{'name_'.app()->currentLocale() }) ? ($r->city->{'name_'.app()->currentLocale() })  : '' }} ( {{$r->city->welayat->{'name_'.app()->currentLocale() } ? ($r->city->welayat->{'name_'.app()->currentLocale() })  : '' }} )</td>
							...
							<td class="table-report__action w-56">
								<div class="flex justify-center items-center">
									<a class="flex items-center mr-3" href="{{route('microdistricts.edit', $r)}}">
		...
		<!-- END: Data List -->
		{!! $microdistricts->links('vendor.pagination.custom') !!}
	</div>
			...
			<div id="delete-confirmation-modal{{$r->id}}" class="modal" tabindex="-1" aria-hidden="true">
				<div class="modal-dialog">
					<div class="modal-content">
						<form  action="{{route('microdistricts.destroy', $r)}}" method="POST" class="modal-body p-0">
							@csrf
							@method('delete')
