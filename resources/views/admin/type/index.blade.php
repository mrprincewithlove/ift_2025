	...
	<div class="grid grid-cols-12 gap-6 mt-5">
		<div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
			...
			<div class="hidden md:block mx-auto text-slate-500">
				{{ __('translates.entries_info', ['start' => $types->firstItem(), 'end' => $types->lastItem(), 'total' => $types->total()]) }}
			</div>
		...
		<div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
			<table class="table table-report -mt-2">
				...
				@if(count($types)>0)
					@foreach($types as $r)
						<tr class="intro-x">
							<td class="text-center">{{ $loop->iteration + $types->firstItem() - 1 }}</td>
							<td class="text-center">{{ $r->{'title_'.app()->currentLocale()} ?? '' }}</td>
							<td class="table-report__action w-56">
								<div class="flex justify-center items-center">
									<a class="flex items-center mr-3" href="{{route('types.edit', $r)}}">
			...
			<div id="delete-confirmation-modal{{$r->id}}" class="modal" tabindex="-1" aria-hidden="true">
				<div class="modal-dialog">
					<div class="modal-content">
						<form  action="{{route('types.destroy', $r)}}" method="POST" class="modal-body p-0">
							@csrf
							@method('delete')
