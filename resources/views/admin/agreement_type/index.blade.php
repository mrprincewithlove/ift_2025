...
@section('content')
	<h2 class="intro-y text-lg font-medium mt-10">
		{{__('translates.agreement_types')}}
	</h2>

	<div class="grid grid-cols-12 gap-6 mt-5">
		<div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
			<a href="{{route('agreement-types.create')}}"
			   class="btn btn-primary shadow-md mr-2">{{ __('translates.add_new') }}</a>

			<div class="hidden md:block mx-auto text-slate-500">
				{{ __('translates.entries_info', ['start' => $agreement_types->firstItem(), 'end' => $agreement_types->lastItem(), 'total' => $agreement_types->total()]) }}
			</div>

			<form action="{{route('agreement-types.index')}}" method="GET"
		...
		<div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
			<table class="table table-report -mt-2">
				...
				<tr>
					...
					<th class="text-center whitespace-nowrap">{{__('translates.period_month')}}</th>
					<th class="text-center whitespace-nowrap">{{__('translates.discount_percent')}}</th>
				...
				<tbody>
				@if(count($agreement_types)>0)
					@foreach($agreement_types as $r)
						<tr class="intro-x">
							<td class="text-center">{{ $loop->iteration + $agreement_types->firstItem() - 1 }}</td>
							<td class="text-center">{{ $r->name_tm ?? '' }}</td>
							<td class="text-center">{{ $r->name_ru ?? '' }}</td>
							<td class="text-center">{{ $r->name_en ?? '' }}</td>
							<td class="text-center">{{ $r->period_month ?? '0' }}</td>
							<td class="text-center">{{ $r->discount_percent ?? '0' }}%</td>
							...
							<td class="table-report__action w-56">
								<div class="flex justify-center items-center">
									<a class="flex items-center mr-3" href="{{route('agreement-types.edit', $r)}}">
		...
		<!-- END: Data List -->
		{!! $agreement_types->links('vendor.pagination.custom') !!}
	</div>
			...
			<div id="delete-confirmation-modal{{$r->id}}" class="modal" tabindex="-1" aria-hidden="true">
				<div class="modal-dialog">
					<div class="modal-content">
						<form  action="{{route('agreement-types.destroy', $r)}}" method="POST" class="modal-body p-0">
							@csrf
							@method('delete')
