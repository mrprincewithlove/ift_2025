...
@section('content')
	<h2 class="intro-y text-lg font-medium mt-10">
		{{__('translates.Number')}}
	</h2>

	<div class="grid grid-cols-12 gap-6 mt-5">
		<div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
			<a href="{{route('numbers.create')}}"
			   class="btn btn-primary shadow-md mr-2">{{ __('translates.add_new') }}</a>

			<div class="hidden md:block mx-auto text-slate-500">
				{{ __('translates.entries_info', ['start' => $numbers->firstItem(), 'end' => $numbers->lastItem(), 'total' => $numbers->total()]) }}
			</div>
		...
		<div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
			<table class="table table-report -mt-2">
				...
				<tr>
					...
					<th class="text-center whitespace-nowrap">{{__('translates.icon')}}</th>
					<th class="text-center whitespace-nowrap">{{__('translates.title_'.app()->currentLocale())}}</th>
					<th class="whitespace-nowrap text-center">{{__('translates.number')}}</th>
				...
				@if(count($numbers)>0)
					@foreach($numbers as $r)
						<tr class="intro-x">
							<td class="text-center">{{ $loop->iteration + $numbers->firstItem() - 1 }}</td>
							<td class="text-center">
								@php
									if (file_exists(public_path($r->icon))) {
								...
								@if($isSvg)
									@php
										echo  '<span class="w-20 h-20 group-hover:scale-105 transition-all">'.file_get_contents(public_path($r->icon)).'</span>';
									@endphp
								@else
									<img src="{{asset($r->icon)}}" alt="" class="w-20 h-20 group-hover:scale-105 transition-all">
							...
							<td class="table-report__action w-56">
								<div class="flex justify-center items-center">
									<a class="flex items-center mr-3" href="{{route('numbers.edit', $r)}}">
		...
		<!-- END: Data List -->
		{!! $numbers->links('vendor.pagination.custom') !!}
	</div>
			...
			<div id="delete-confirmation-modal{{$r->id}}" class="modal" tabindex="-1" aria-hidden="true">
				<div class="modal-dialog">
					<div class="modal-content">
						<form  action="{{route('numbers.destroy', $r)}}" method="POST" class="modal-body p-0">
							@csrf
							@method('delete')
