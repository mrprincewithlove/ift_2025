...
{{--               class="btn btn-primary shadow-md mr-2">{{ __('translates.add_new_permission') }}</a>--}}
            <form action="{{route('visas.excell')}}" method="GET">
                <button type="submit" class="btn btn-primary shadow-md mr-2">{{ __('translates.excell') }}</button>
            </form>

            <div class="hidden md:block mx-auto text-slate-500">
                {{ __('translates.entries_info', ['start' => $visas->firstItem(), 'end' => $visas->lastItem(), 'total' => $visas->total()]) }}
            </div>
        ...
        <div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
            <table class="table table-report -mt-2">
                ...
                <tr>
                    ...
                    <th class="whitespace-nowrap text-center">{{__('translates.full_name')}}</th>
                    <th class="text-center whitespace-nowrap">{{__('translates.born')}}</th>
                    <th class="text-center whitespace-nowrap">{{__('translates.citizen')}}</th>
                    <th class="text-center whitespace-nowrap">{{__('translates.passport')}}</th>
                    <th class="text-center whitespace-nowrap">{{__('translates.education')}}</th>
                    <th class="text-center whitespace-nowrap">{{__('translates.visa')}}</th>
                ...
                @if(count($visas)>0)
                    @foreach($visas as $r)
                        <tr class="intro-x">
                            <td class="text-center">{{ $loop->iteration + $visas->firstItem() - 1 }}</td>
                            <td class="text-center">{{ $r->surname ?? '' }} {{ $r->middle_name ?? '' }} {{ $r->name ?? '' }}</td>
                            <td class="text-center">{{ $r->born_date ?? '' }} {{ $r->born_place ?? '' }}</td>
                            <td class="text-center">{{ $r->citizen ?? '' }}</td>
                            <td class="text-center">{{ $r->passport_number ?? '' }} {{ $r->passport_date ?? '' }}</td>
                            <td class="text-center">{{ $r->education ?? '' }}</td>
                            <td class="text-center">{{ $r->visa_date ?? '' }} {{ $r->coming_text ?? '' }}</td>
