...
@section('content')
    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="intro-y col-span-12 lg:col-span-12">
            <!-- BEGIN: Form Layout -->
            <form action="{{route('permissions.store')}}" method="POST" class="intro-y box p-5">
                @csrf
                ...
                <div class="text-right mt-5">
                    ...
                    <a href="{{ Session::get('prev-url') ?? URL::previous() }}" class="btn btn-outline-secondary  mr-1 whitespace-nowrap">{{__('permission.back')}}</a>
                    <button type="submit" class="btn btn-primary whitespace-nowrap">{{__('permission.save')}}</button>
                </div>
            </form>
            <!-- END: Form Layout -->
        </div>
