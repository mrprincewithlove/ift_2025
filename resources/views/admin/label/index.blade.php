	...
	<div class="grid grid-cols-12 gap-6 mt-5">
		<div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
			...
			<div class="hidden md:block mx-auto text-slate-500">
				{{ __('translates.entries_info', ['start' => $labels->firstItem(), 'end' => $labels->lastItem(), 'total' => $labels->total()]) }}
			</div>

			<form action="{{route('labels.index')}}" method="GET"
		...
		<div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
			<table class="table table-report -mt-2">
				...
				<tr>
					...
					<th class="whitespace-nowrap text-center">{{__('translates.color')}}</th>
				...
				@if(count($labels)>0)
					@foreach($labels as $r)
						<tr class="intro-x">
							<td class="text-center">{{ $loop->iteration + $labels->firstItem() - 1 }}</td>
							<td class="text-center">{{ $r->{'title_'.app()->currentLocale()} ?? '' }}</td>
							<td class="text-center">{{ $r->color ?? '' }}</td>
							<td class="table-report__action w-56">
								<div class="flex justify-center items-center">
									<a class="flex items-center mr-3" href="{{route('labels.edit', $r)}}">
		...
		<!-- END: Data List -->
		{!! $labels->links('vendor.pagination.custom') !!}
	</div>
			...
			<div id="delete-confirmation-modal{{$r->id}}" class="modal" tabindex="-1" aria-hidden="true">
				<div class="modal-dialog">
					<div class="modal-content">
						<form  action="{{route('labels.destroy', $r)}}" method="POST" class="modal-body p-0">
							@csrf
							@method('delete')
