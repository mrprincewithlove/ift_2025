	...
	<div class="grid grid-cols-12 gap-6 mt-5">
		<div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
			...
			<div class="hidden md:block mx-auto text-slate-500">
				{{ __('translates.entries_info', ['start' => $agreements->firstItem(), 'end' => $agreements->lastItem(), 'total' => $agreements->total()]) }}
			</div>

			<form action="{{route('agreements.index')}}" method="GET"
		...
		<div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
			<table class="table table-report -mt-2">
				...
				<tr>
					...
					<th class="text-center whitespace-nowrap">{{__('translates.No')}}</th>
					<th class="text-center whitespace-nowrap">{{__('translates.client')}}</th>
					<th class="text-center whitespace-nowrap">{{__('translates.agreement_price')}}</th>
					<th class="text-center whitespace-nowrap">{{__('translates.agreement_type')}}</th>
				...
				@if(count($agreements)>0)
					@foreach($agreements as $r)
						<tr class="intro-x">
							<td class="text-center">{{ $loop->iteration + $agreements->firstItem() - 1 }}</td>
							...
							<td class="text-center">{{ $r->client->getFullName() ?? '' }}</td>
							<td class="text-center">{{ $r->agreement_price ?? '' }}</td>
							<!-- <td class="text-center">{{ $r->center->{'name_'.app()->currentLocale()} ?? '' }}</td> -->
							<td class="text-center">{{ $r->agreement_type->{'name_'.app()->currentLocale()} ?? '' }}</td>
							<td class="text-center">{{ $r->agreement_status->{'name_'.app()->currentLocale()} ?? '' }}</td>
							
							<td class="table-report__action w-56">
								<div class="flex justify-center items-center">
                                    <a class="flex items-center mr-3" href="{{route('agreement.invoices', $r)}}">
                                        <i data-lucide="file-digit" class="w-4 h-4"></i>
                                        {{__('translates.invoices_short')}} </a>
									<a class="flex items-center mr-3" href="{{route('agreement.payments', $r)}}">
										<i data-lucide="banknote" class="w-4 h-4 la la-print"></i>
										{{__('translates.payments')}} </a>
									<a id="print" class="flex items-center mr-3" href="{{route('agreement-print', $r)}}">
										<i data-lucide="printer" class="w-4 h-4 la la-print"></i>
										{{__('translates.print')}} </a>

                                    <!-- <a class="flex items-center mr-3" href="{{route('agreements.edit', $r)}}">
			...
			<div id="delete-confirmation-modal{{$r->id}}" class="modal" tabindex="-1" aria-hidden="true">
				<div class="modal-dialog">
					<div class="modal-content">
						<form  action="{{route('agreements.destroy', $r)}}" method="POST" class="modal-body p-0">
							@csrf
							@method('delete')
							...
							<div class="p-5 text-center">
								...
								<div class="text-slate-500 mt-2">
									...
									<br>
									{{__('translates.This process deletes agreement and its payments invoices and comments')}}
