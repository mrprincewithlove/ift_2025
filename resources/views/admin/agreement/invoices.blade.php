	...
	<div class="grid grid-cols-12 gap-6 mt-5">
		<div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2 justify-between">
			...
			<div class="hidden md:block mx-auto text-slate-500">
				{{ __('translates.entries_info', ['start' => $invoices->firstItem(), 'end' => $invoices->lastItem(), 'total' => $invoices->total()]) }}
			</div>
		...
		<div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
			<table class="table table-report -mt-2">
				...
				<tr>
					...
					<th class="text-center whitespace-nowrap">{{__('translates.invoice_no')}}</th>					
					<th class="whitespace-nowrap text-center">{{__('translates.date')}}</th>
					<th class="whitespace-nowrap text-center">{{__('translates.client')}}</th>
					<th class="text-center whitespace-nowrap">{{__('translates.price') }}</th>
					<th class="text-center whitespace-nowrap">{{__('translates.paid') }}</th>
					<th class="text-center whitespace-nowrap">{{__('translates.status')}}</th>
					<th class="text-center whitespace-nowrap">{{__('translates.actions')}}</th>
				</tr>
				</thead>
				<tbody>
				@if(count($invoices)>0)
					...
					<tr class="intro-x">
							...
							<td class="text-center">{{ ($r->agreement->client) ? $r->agreement->client->surname . ' ' . $r->agreement->client->name . ' ' . $r->agreement->client->father_name : '' }}</td>
							<td class="text-center">{{ ($r->price) ? $r->price . 'tmt' : '' }}</td>
							<td class="text-center">{{ ($r->particular_payment) ? $r->particular_payment . 'tmt' : '' }}</td>
							<td class="text-center">
								...
								@elseif($r->status == 'prepayment')
									<div class="flex items-center justify-center text-success">
										<i data-lucide="check-square" class="w-6 h-6"></i>
										{{ __('translates.prepayment') }}
									</div>
								@elseif($r->status == 'particular')
									<div class="flex items-center justify-center text-success">
										<i data-lucide="check-square" class="w-6 h-6"></i>
										{{ __('translates.particular') }}
									</div>
								@else
									<div class="flex items-center justify-center text-danger">
										<i data-lucide="check-square" class="w-6 h-6"></i>
										{{ __('translates.not_paid') }}
									</div>
								@endif
							</td>
							<td class="table-report__action w-56">
								<div class="flex justify-center items-center">
									<a id="print" class="flex items-center mr-3" href="{{route('invoice-print', $r)}}">
										<i data-lucide="printer" class="w-4 h-4 la la-print"></i>
										{{__('translates.print')}} </a>
									<!-- <a class="flex items-center mr-3" href="{{route('categories.edit', $r)}}">
			...
			<div id="delete-confirmation-modal{{$r->id}}" class="modal" tabindex="-1" aria-hidden="true">
				<div class="modal-dialog">
					<div class="modal-content">
						<form  action="{{route('cities.destroy', $r)}}" method="POST" class="modal-body p-0">
							@csrf
							@method('delete')
