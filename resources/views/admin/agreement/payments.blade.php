...
@section('content')
	<h2 class="intro-y text-lg font-medium mt-10">
		{{__('translates.payments')}}
	</h2>

	<div class="grid grid-cols-12 gap-6 mt-5">
		<div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
		<a href="{{route('agreement.payments.create', $agreement)}}"
			   class="btn btn-primary shadow-md mr-2">{{ __('translates.add_new') }}</a>
			<div class="px-3 py-2 border rounded-md bg-slate-200 flex items-center justify-between gap-4">
			<p>{{__('translates.Payments to end agreement')}} :</p>
			<p>{{$agreement->price_to_pay . 'tmt'}}</p>

			</div>

			<div class="hidden md:block mx-auto text-slate-500">
				{{ __('translates.entries_info', ['start' => $payments->firstItem(), 'end' => $payments->lastItem(), 'total' => $payments->total()]) }}
			</div>

			{{--<form action="{{route('agreement.payments', $agreement)}}" method="GET"--}}
		...
		<div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
			<table class="table table-report -mt-2">
				...
				<th class="whitespace-nowrap text-center">#</th>
					<th class="text-center whitespace-nowrap">{{__('translates.payment_no')}}</th>
					<th class="whitespace-nowrap text-center">{{__('translates.date')}}</th>
					<th class="text-center whitespace-nowrap">{{__('translates.price')}}</th>
					<th class="text-center whitespace-nowrap">{{__('translates.agreement')}}</th>
					<th class="text-center whitespace-nowrap">{{__('translates.client')}}</th>
				...
				@if(count($payments)>0)
					@foreach($payments as $r)
						<tr class="intro-x">
						<td class="text-center">{{ $loop->iteration + $payments->firstItem() - 1 }}</td>
							<td class="text-center">
								<div class="flex flex-col	">
									<p>{{ $r->payment_no ?? '' }}</p>
									<p>{{__('translates.check_number') .' : '.   $r->check_number ?? '' }}</p>
...
{{--								{{ $r->payment_no ?? '' }}--}}
							...
							<td class="text-center">{{ ($r->agreement) ? $r->agreement->agreement_no  : '' }}</td>
							<td class="text-center">{{ ($r->agreement && $r->agreement->client) ? $r->agreement->client->surname . ' ' . $r->agreement->client->name  : '' }}</td>							
							
							<td class="table-report__action w-56">
								<div class="flex justify-center items-center">
									<a id="print" class="flex items-center mr-3" href="{{route('payment-print', $r)}}">
										<i data-lucide="printer" class="w-4 h-4 la la-print"></i>
										{{__('translates.print')}} </a>
									<a class="flex items-center mr-3" href="{{route('payments.edit', $r)}}">
							...
							<td class="text-end">{{ $r->center->{'name_'.app()->currentLocale()} ?? '' }}</td>
			...
			<div id="delete-confirmation-modal{{$r->id}}" class="modal" tabindex="-1" aria-hidden="true">
				<div class="modal-dialog">
					<div class="modal-content">
						<form  action="{{route('payments.destroy', $r)}}" method="POST" class="modal-body p-0">
							@csrf
							@method('delete')
