    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
            <a href="{{route('menus.create')}}"
               class="btn btn-primary shadow-md mr-2">{{ __('translates.add_new_menu') }}</a>

            <div class="hidden md:block mx-auto text-slate-500">
                {{ __('translates.entries_info', ['start' => $menus->firstItem(), 'end' => $menus->lastItem(), 'total' => $menus->total()]) }}
            </div>
        ...
        <div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
            <table class="table table-report -mt-2">
                ...
                @if(count($menus)>0)
                    @foreach($menus as $r)
                        <tr class="intro-x">
                            <td class="text-center">{{ $loop->iteration + $menus->firstItem() - 1 }}</td>
                            <td class="text-center">{{ $r->name ?? '' }}</td>
                            <td class="text-center">{!!  $r->description ?? '' !!}</td>
                            <td class="text-center">{{ $r->created_at ?? '' }}</td>
                            <td class="table-report__action w-56">
                                <div class="flex justify-center items-center">
                                    <a class="flex items-center mr-3" href="{{route('menus.edit', $r->id)}}">
            ...
            <div id="delete-confirmation-modal{{$r->id}}" class="modal" tabindex="-1" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <form  action="{{route('menus.destroy', $r)}}" method="POST" class="modal-body p-0">
                            @csrf
                            @method('delete')
