	...
	<div class="grid grid-cols-12 gap-6 mt-5">
		<div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
			...
			<div class="hidden md:block mx-auto text-slate-500">
				{{ __('translates.entries_info', ['start' => $guarantors->firstItem(), 'end' => $guarantors->lastItem(), 'total' => $guarantors->total()]) }}
			</div>

			<form action="{{route('guarantors.index')}}" method="GET"
		...
		<div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
			<table class="table table-report -mt-2">
				...
				<tr>
					...
					<th class="text-center whitespace-nowrap">{{__('translates.full_name')}}</th>
					<th class="whitespace-nowrap text-center">{{__('translates.work_info')}}</th>
					<th class="text-center whitespace-nowrap">{{__('translates.phone')}}</th>
				...
				@if(count($guarantors)>0)
					@foreach($guarantors as $r)
						<tr class="intro-x">
							<td class="text-center">{{ $loop->iteration + $guarantors->firstItem() - 1 }}</td>
							<td class="text-center">{{ $r->getFullName() ?? '' }}</td>
							...
							<td class="text-center">
								<div class="flex flex-col">
									<p>{{ $r->mobile ? __('translates.mobile').': '.$r->mobile :'' }}</p>
									<p>{{ $r->home_phone ?  __('translates.home_phone').': '.$r->home_phone  :'' }}</p>
									<p>{{ $r->work_phone ? __('translates.work_phone').': '.$r->work_phone   :'' }}</p>
								</div>
							<td class="text-center">
								@if($r->status)
							...
							<td class="table-report__action w-56">
								<div class="flex justify-center items-center">
									<a class="flex items-center mr-3" href="{{route('guarantors.edit', $r)}}">
		...
		<!-- END: Data List -->
		{!! $guarantors->links('vendor.pagination.custom') !!}
	</div>
			...
			<div id="delete-confirmation-modal{{$r->id}}" class="modal" tabindex="-1" aria-hidden="true">
				<div class="modal-dialog">
					<div class="modal-content">
						<form  action="{{route('guarantors.destroy', $r)}}" method="POST" class="modal-body p-0">
							@csrf
							@method('delete')
