	...
	<div class="grid grid-cols-12 gap-6 mt-5">
		<div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
			...
			<div class="hidden md:block mx-auto text-slate-500">
				{{ __('translates.entries_info', ['start' => $translations->firstItem(), 'end' => $translations->lastItem(), 'total' => $translations->total()]) }}
			</div>
		...
		<div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
			<table class="table table-report -mt-2">
				...
				<tr>
					...
					<th class="text-center whitespace-nowrap">{{__('mytr.key')}}</th>
					<th class="text-center whitespace-nowrap">{{__('mytr.file')}}</th>
					<th class="whitespace-nowrap text-center">{{__('mytr.translate_tm')}}</th>
					<th class="text-center whitespace-nowrap">{{__('mytr.translate_ru')}}</th>
					<th class="text-center whitespace-nowrap">{{__('mytr.translate_en')}}</th>
					<th class="text-center whitespace-nowrap">{{__('mytr.action')}}</th>
				</tr>
				</thead>
				<tbody>
				@if(count($translations)>0)
					@foreach($translations as $r)
						<tr class="intro-x">
							<td class="text-center">{{ $loop->iteration + $translations->firstItem() - 1 }}</td>
							<td class="text-center">{{ $r->key ?? '' }}</td>
							<td class="text-center">{{ $r->file ?? '' }}</td>
							<td class="text-center">{{ $r->value_tm ?? '' }}</td>
							<td class="text-center">{{ $r->value_ru ?? '' }}</td>
							<td class="text-center">{{ $r->value_en ?? '' }}</td>
							<td class="table-report__action w-56">
								<div class="flex justify-center items-center">
									<a class="flex items-center mr-3" href="{{route('translations.edit', $r)}}">
			...
			<div id="delete-confirmation-modal{{$r->id}}" class="modal" tabindex="-1" aria-hidden="true">
				<div class="modal-dialog">
					<div class="modal-content">
						<form  action="{{route('translations.destroy', $r)}}" method="POST" class="modal-body p-0">
							@csrf
							@method('delete')
