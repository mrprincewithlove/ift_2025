...
@section('content')
	<h2 class="intro-y text-lg font-medium mt-10">
		{{__('translates.SocialMedia')}}
	</h2>

	<div class="grid grid-cols-12 gap-6 mt-5">
		<div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
			<a href="{{route('socialMedias.create')}}"
			   class="btn btn-primary shadow-md mr-2">{{ __('translates.add_new') }}</a>

			<div class="hidden md:block mx-auto text-slate-500">
				{{ __('translates.entries_info', ['start' => $socialMedias->firstItem(), 'end' => $socialMedias->lastItem(), 'total' => $socialMedias->total()]) }}
			</div>

			<form action="{{route('socialMedias.index')}}" method="GET"
		...
		<div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
			<table class="table table-report -mt-2">
				...
				@if(count($socialMedias)>0)
					@foreach($socialMedias as $r)
						<tr class="intro-x">
							<td class="text-center">{{ $loop->iteration + $socialMedias->firstItem() - 1 }}</td>
							...
							<td class="table-report__action w-56">
								<div class="flex justify-center items-center">
									<a class="flex items-center mr-3" href="{{route('socialMedias.edit', $r)}}">
		...
		<!-- END: Data List -->
		{!! $socialMedias->links('vendor.pagination.custom') !!}
	</div>
			...
			<div id="delete-confirmation-modal{{$r->id}}" class="modal" tabindex="-1" aria-hidden="true">
				<div class="modal-dialog">
					<div class="modal-content">
						<form  action="{{route('socialMedias.destroy', $r)}}" method="POST" class="modal-body p-0">
							@csrf
							@method('delete')
