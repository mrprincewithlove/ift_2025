...
class ActivityLogController extends Controller
{
    ...
    {
        $r = $this->hasPermission('activitylog', auth()->user()->role_id);
        if(!$r)
            return back()->with('prev-url', request()->input('prev-url') ?? '')->with('permission', 'no permission');

        $menu = [ 'Configs', 'SystemLog' ];
        if($page == 'page')
        {
            $input['log_name'] = ['default', 'specific'];
            $input['description'] = ['created', 'updated', 'deleted'];
            $input['subjects'] = ['Permission', 'Teammate', 'SocialMedia', 'Slider', 'Center', 'Information', 'Post', 'Role', 'Tariff', 'User'];
            $input['users'] = User::get(['id', 'name', 'surname']);
            return view('admin.config.activitylog', compact('menu'))->with('input', $input);
        }
        ...
        $row = $row->select(ActivityLog::getColumns());
        ...
        $view = view('admin.config.activitylog-table')->with('activitylogs', $row)->with('input', $input)->render();
    ...
    public function getActivityLogProperty()
    {
        if(request()->ajax())
            if(request()->has('log_id'))
            {
                ...
                if($activityLog)
                    return response()->json(['data' => view('admin.config.activitylog-column')->with('activityLog', $activityLog)->render()]);
            }
